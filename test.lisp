...
(deftest connection ()
  ...
  (should be string= "Hello World!"
          (with-connection () (red-echo "Hello World!")))
  (should signal redis-error-reply
          (with-connection () (red-auth "pass"))))
;; QUIT - futile


(deftest *-commands (:wrap with-test-db)
  (should be string= "OK"
          (red-set "mykey" "Hello"))
  (should be true
          (red-expire "mykey" 10))
...
(deftest sort (:wrap with-test-db)
  (should be true
          (red-rpush "numbers" "1"))
  (should be true
          (red-rpush "числа" "1"))
...
(deftest str-commands (:wrap with-test-db)
  (should be true
         (red-set "y" "1"))
  (should be true
         (red-set "ігрек" "1"))
...
(deftest l-commands (:wrap with-test-db)
  (should be = 1
          (red-rpush "l" "1"))
  (should be = 1
          (red-rpush "эл" "1"))
...
(deftest s-commands (:wrap with-test-db)
  (should be true
          (red-sadd "s" "1"))
  (should be true
          (red-sadd "э" "1"))
...
(deftest z-commands (:wrap with-test-db)
  (should be true
          (red-zadd "set" 1 "e1"))
  (should be true
          (red-zadd "множина"
...
(deftest h-commands (:wrap with-test-db)
  (should be true
          (red-hset "h1" "f1" "a"))
  (should be true
          (red-hset "h1" "f2" "b"))
...
(deftest transactions (:wrap with-test-db)
  (should be string= "OK"
          (red-multi))
...
(deftest pubsub (:wrap with-test-db)
...
(deftest pipelining (:wrap with-test-db)
...
(deftest server (:wrap with-test-db)
  (should be true
          (red:save))
  (should be true
          (red:bgsave))
...
(deftest scripting (:wrap with-test-db
